...
def main():
    ...
    #添加对象
    parser.add_argument('--opt', type=str, default='C:/Users/yxx/Desktop/FFTLOL/options/train/train_Enhance.yml',
    ...
    #返回  train——Enhance.yml列表
    opt = option.parse(args.opt, is_train=True) # 训练 / 测试
    ...
    if rank <= 0:  # normal training (rank -1) OR distributed training (rank 0)
        if resume_state is None:
            ...
            for key, path in opt['path'].items():
                if key != 'resume_state' and key != 'pretrain_model_G':
                    mk=key
                    mk1=path
                    util.mkdir(path)
        ...
        logger = logging.getLogger('base') # 创建日志器对象
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            ...
            train_size = int(math.ceil(len(train_set) / dataset_opt['batch_size'])) # 向上取整”， 即小数部分直接舍去，并向正数部分进1 485 / 4 =122
            total_iters = int(opt['train']['niter']) # 50000
            total_epochs = int(math.ceil(total_iters / train_size)) # 50000 / 122 = 410
    ...
    # # #
        for batch_step, train_data in enumerate(train_loader):
            ...
            model.optimize_parameters(current_step) # 前向传播
        ...
        if opt['datasets'].get('val', None) and epoch % opt['train']['val_epoch'] == 0:
            ...
            for val_data in val_loader:
                ...
                if epoch % opt['logger']['save_checkpoint_epoch'] == 0:
                    img_dir = os.path.join(opt['path']['root'], 'experiment/STEN/val_images', str(epoch))
